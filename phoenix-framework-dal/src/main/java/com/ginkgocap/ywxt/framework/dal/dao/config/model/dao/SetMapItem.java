    ...
    public void setObjectName(String objectName) {
        ...
        if (StringUtils.isNotEmpty(objectName) && StringUtils.isNotEmpty(keyProperty)
            ...
            try {
                ...
                String[] values = StringUtils.splitPreserveAllTokens(this.valueProperty, DaoConfig.PROPERTY_SEPARATOR);
                this.valueMethod = new Method[values.length];
                for (int i = 0; i < values.length; i++) {
                    valueMethod[i] = MethodUtils.getAccessibleMethod(Class.forName(objectName), "get"
                            + WordUtils.capitalize(values[i]), new Class[] {});
                }
